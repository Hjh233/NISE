...
from torch_rechub.models.multi_task import DCN4ESMM, DCN4DCMT, DCN4DR
from torch_rechub.trainers import DCNTrainer
...
def train(party, seed, weight, strategy, ablation_weight, device, user_features, item_features, train_dataloader, test_dataloader, cvr_params, counterfactual_cvr_params, ctr_params):
    if 'dcn' in strategy and 'dcmt' in strategy:
        model = DCN4DCMT(user_features, item_features, cvr_params, counterfactual_cvr_params, ctr_params)
        task_types = ["classification", "classification", "classification", "classification"] 
    elif 'dcn' in strategy and 'dr' in strategy:
        model = DCN4DR(user_features, item_features, cvr_params, ctr_params, imputation_params)
        task_types = ["classification", "classification", "classification", "regression"] 
    elif 'dcn' in strategy:
        model = DCN4ESMM(user_features, item_features, cvr_params, ctr_params)
    ...
    save_dir = 'logs/ali_ccp/local/rebuttal/dcn/{}'.format(strategy)
    ...
    total_log = dcn_trainer.fit(train_dataloader, test_dataloader, party, seed, weight, strategy, ablation_weight)
...
def inference(party, strategy, device, user_features, item_features, test_dataloader, cvr_params, counterfactual_cvr_params, ctr_params):
    ...
    f = open(save_dir + '/log_loss.txt', 'a')
    ...
    for seed in range(5):
        ...
        model.load_state_dict(torch.load('logs/ali_ccp/local/dcn/{}/{}/model_both_{}.pth'.format(strategy, party, seed)))
        ...
        _log_loss = dcn_trainer.evaluate_log_loss(test_dataloader)

        print('CVR LogLoss:', _log_loss)

        f.write('{}'.format(_log_loss))
        f.write(',')
