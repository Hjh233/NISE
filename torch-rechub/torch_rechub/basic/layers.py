...
class DCN4CVR(torch.nn.Module):
    ...
    def __init__(self,
        ...
        super(DCN4CVR, self).__init__()
        self.input_dims = input_dims
        if use_low_rank_mixture:
            self.crossnet = CrossNetMix(self.input_dims, n_cross_layers, low_rank=low_rank, num_experts=num_experts)
        else:
            self.crossnet = CrossNetV2(self.input_dims, n_cross_layers)
        self.model_structure = model_structure

        if self.model_structure == "stacked":
            self.stacked_dnn = MLP(self.input_dims,
                                   output_layer=False,
                                   ** mlp_params)
            final_dim = mlp_params["dims"][-1]
        if self.model_structure == "parallel":
            self.parallel_dnn =  MLP(self.input_dims,
                                     output_layer = False,
                                   ** mlp_params)
            final_dim = mlp_params["dims"][-1] + self.input_dims
        if self.model_structure == "crossnet_only": # only CrossNet
            final_dim = self.input_dims
...
class DCN4FeatureExtraction(torch.nn.Module):
    ...
    def __init__(self,
        ...
        super(DCN4FeatureExtraction, self).__init__()
        self.input_dims = input_dims
        if use_low_rank_mixture:
            self.crossnet = CrossNetMix(self.input_dims, n_cross_layers, low_rank=low_rank, num_experts=num_experts)
        else:
        ...
        if self.model_structure == "parallel":
            self.parallel_dnn =  MLP(self.input_dims,
                                     output_layer = False,
                                     ** expert_params)
        linear_input_dims = expert_params['dims'][-1]
        self.linear = nn.Linear(linear_input_dims, linear_input_dims, bias=True)
    ...
    def forward(self, x):
        ...
        if self.model_structure == "crossnet_only":
            ...
            dnn_out = self.parallel_dnn(x)
            final_out = torch.cat([cross_out, dnn_out], dim=1)
        embeddings = self.linear(final_out)

        return embeddings


class DeepFM4CVR(torch.nn.Module):

    def __init__(self, feature_dims, mlp_params):
        super(DeepFM4CVR, self).__init__()
        self.feature_dims = feature_dims
        self.linear = LR(self.feature_dims)  # 1-odrder interaction
        self.fm = FM(reduce_sum=True)  # 2-odrder interaction
        self.mlp = MLP(self.feature_dims, **mlp_params)

    def forward(self, x, x_fm):
        y_linear = self.linear(x)
        y_fm = self.fm(x_fm)
        y_deep = self.mlp(x)  #[batch_size, 1]
        y = y_linear + y_fm + y_deep
        return torch.sigmoid(y)
  

class DeepFM4FeatureExtraction(torch.nn.Module):

    def __init__(self, feature_dims, emb_dim_per_feature):
        super(DeepFM4FeatureExtraction, self).__init__()
        self.feature_dims = feature_dims
        self.linear = nn.Linear(feature_dims, emb_dim_per_feature, bias=True)  # 1-odrder interaction
        self.fm = FM(reduce_sum=False)  # 2-odrder interaction
        self.mlp = MLP(self.feature_dims, output_layer=False, **{"dims":[emb_dim_per_feature]})

    def forward(self, x, x_fm):
        emb_linear = self.linear(x)
        emb_fm = self.fm(x_fm)
        emb_deep = self.mlp(x)  
        embeddings = emb_linear + emb_fm + emb_deep
