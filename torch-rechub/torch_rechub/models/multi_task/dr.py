...
class DR(nn.Module):
    ...
    def __init__(self, user_features, item_features, cvr_params, ctr_params, imputation_params):
        super().__init__()
        ...
        self.tower_imputation = MLP(self.tower_dims, **imputation_params)
    ...
    def forward(self, x):
        ...
        imputation_logit = self.tower_imputation(input_tower)
        cvr_pred = torch.sigmoid(cvr_logit)
        ctr_pred = torch.sigmoid(ctr_logit)
        ctcvr_pred = torch.mul(ctr_pred, cvr_pred)
        imputation_pred = torch.sigmoid(imputation_logit) # imputation error \in [0,1]
        ...
        clipped_imputation_pred = torch.clamp(imputation_pred, min=clip_min-1, max=clip_max)

        ys = [cvr_pred, ctr_pred, ctcvr_pred]
        clipped_ys = [clipped_cvr_pred, clipped_ctr_pred, ctcvr_pred, clipped_imputation_pred]
