...
class ConcatTorchDataset(Dataset):
    ...
    def __init__(self, x_advertiser, y_advertiser, x_platform, y_platform):
        super().__init__()
        self.x_advertiser = x_advertiser
        self.y_advertiser = y_advertiser
        self.x_platform = x_platform
        self.y_platform = y_platform

    def __getitem__(self, index):
        return {k: v[index] for k, v in self.x_advertiser.items()}, self.y_advertiser[index], {k: v[index] for k, v in self.x_platform.items()}, self.y_platform[index]

    def __len__(self):
        return len(self.y_advertiser)
...
class DataGenerator(object):
    ...
    def generate_dataloader(self, 
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, worker_init_fn=worker_init_fn)
        val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, worker_init_fn=worker_init_fn)
        test_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, worker_init_fn=worker_init_fn)

        return train_dataloader, val_dataloader, test_dataloader


class ConcatDataGenerator(object):

    def __init__(self, x_advertiser, y_advertiser, x_platform, y_platform):
        super().__init__()
        self.concat_dataset = ConcatTorchDataset(x_advertiser, y_advertiser, x_platform, y_platform)
        self.concat_length = len(self.concat_dataset)

    def generate_dataloader(self, 
                            x_val_advertiser=None, y_val_advertiser=None, x_test_advertiser=None, y_test_advertiser=None, 
                            x_val_platform=None, y_val_platform=None, x_test_platform=None, y_test_platform=None, 
                            batch_size=16, num_workers=8,
                            persistent_wokers=True
                            # , worker_init_fn=worker_init_fn
                            ):

        concat_train_dataset = self.concat_dataset
        concat_val_dataset = ConcatTorchDataset(x_val_advertiser, y_val_advertiser, x_val_platform, y_val_platform)
        concat_test_dataset = ConcatTorchDataset(x_test_advertiser, y_test_advertiser, x_test_platform, y_test_platform)

        concat_train_dataloader = DataLoader(concat_train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, persistent_workers=persistent_wokers)
        concat_val_dataloader = DataLoader(concat_val_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, persistent_workers=persistent_wokers)
        concat_test_dataloader = DataLoader(concat_test_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, persistent_workers=persistent_wokers)

        return concat_train_dataloader, concat_val_dataloader, concat_test_dataloader
