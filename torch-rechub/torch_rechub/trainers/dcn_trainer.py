...
from ..models.multi_task import DCN4ESMM, DCN4DCMT, DCN4DR
...
class DCNTrainer(object):
    ...
    def train_one_epoch(self, data_loader, weight, strategy, ablation_weight, epoch=-1, loss_history=[], kappa_history=[]):
        ...
        for iter_i, (x_dict, ys) in enumerate(tk0):
            ...
            if isinstance(self.model, DCN4ESMM):
                if 'esmm' in strategy:
            ...
            elif isinstance(self.model, DCN4DCMT):
                '''
            ...
            elif isinstance(self.model, DCN4DR):
                '''
