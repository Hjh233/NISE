...
from ..models.multi_task import DeepFM4ESMM, DeepFM4DCMT, DeepFM4DR
...
class DeepFMTrainer(object):
    ...
    def train_one_epoch(self, data_loader, weight, strategy, ablation_weight, epoch=-1, loss_history=[], kappa_history=[]):
        temperature = 2.0
        alpha = 0.5
        ...
        for iter_i, (x_dict, ys) in enumerate(tk0):
            ...
            if isinstance(self.model, DeepFM4ESMM):
                if 'esmm' in strategy:
                ...
                if 'ablation_1' in strategy: # only cvr model
                    clicked_cvr_weight = ys[:, 1] 
                    unclicked_cvr_weight = 1 - ys[:, 1]
                ...
                if 'ablation_3' in strategy: # unclicked samples
                    ...
                    cvr_weight_per_batch = loss_ctr.item() / loss_cvr.item()
                    cvr_weight_per_batch = min(cvr_weight_per_batch, 50)
                    loss = loss_ctr + cvr_weight_per_batch * loss_cvr

                    loss_list = [loss_cvr, loss_ctr, loss_ctcvr]


            elif isinstance(self.model, DeepFM4DCMT):
                '''
            ...
            elif isinstance(self.model, DeepFM4DR):
                '''
